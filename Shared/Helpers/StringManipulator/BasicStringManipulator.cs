...
Overview:   Contains string manipulators that handle basic string values.
...
namespace AlchemicalFlux.Utilities.Helpers
{
    ...
    public class BasicStringManipulator : BaseStringManipulator
    {
        ...
        /// </remarks>   
        public override string MultipleReplace(string originalText, 
            Dictionary<string, string> replacements)
        {
            if(AreParametersInvalid(originalText, replacements, 
                nameof(originalText), nameof(replacements)))
            {
                return originalText;
            }

            // Escape any special characters in the dictionary keys and order by
            // longest, then alphabetical.
            var escapedKeys = 
                replacements.Keys
                .Select(key => $"(?<{key.Length}>{Regex.Escape(key)})").
                OrderByDescending(key => key.Length).
                ThenBy(key => key);

            // Concatenates all keys to a searchable pattern. Upon finding an
            // entry to replace, it attempts to match with key / value in the
            // replacements.
            return Regex.Replace(originalText,
                ...
                delegate (Match m)
                {
                    ...
                    if(replacements.TryGetValue(m.Value, 
